...
void jacobi_method (int n, double **A, double *b, double epsilon, int maxit, int *numit, double *x );
...
int main ( )
{
    ...
    jacobi_method(size,A,b,eps,maxiter,&cnt,x);
...
void jacobi_method (int n, double **A, double *b, double epsilon, int maxiter, int *numit, double *x){
   double *dx,*y;
   ...
   for(k = 0; k < maxiter; k++){
      double sum = 0.0;
      for(i = 0; i < n; i++){
         dx[i] = b[i];	// Final Desired Solution: all-ones vector
         for(j = 0; j < n; j++)
            dx[i] -= A[i][j]*x[j];     
		 dx[i] /= A[i][i];
         y[i] += dx[i];
